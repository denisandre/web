/*
 * FileUpload GeneXus User Control 0.0.0.172852
 *
...
gx.getMessage("GXM_fileupload_drophere"),'\x3c/span\x3e\n\x3c/div\x3e\n\x3ctable role\x3d"presentation" class\x3d"table table-striped"\x3e\x3ctbody class\x3d"files"\x3e\x3c/tbody\x3e\x3c/table\x3e\n\x3c/div\x3e'].join("\n"));l.append(g);g.fileupload({url:location.pathname.substring(0,location.pathname.lastIndexOf("/")+1)+this.UploadURL+"/gxobject",autoUpload:m(this.AutoUpload)});g.bind("fileuploadadded",function(){v++}.closure(this));g.bind("fileuploaddestroyed",function(a,b){var c=b.context.data("index");
f[c].destroyed=!0});g.bind("fileuploadcompleted",function(){d||(e=a);d=!0;a++;a===e+v&&this.UploadComplete&&(this.UploadComplete(),v=0,d=!1,e=0)}.closure(this));g.bind("fileuploadfailed",function(a,d){h++;c.each(d.files,function(a,c){b.push(c)});this.UploadFailed&&this.UploadFailed()}.closure(this));l={disableImageResize:m(this.DisableImageResize),maxFileSize:this.MaxFileSize,getFilesFromResponse:function(b){for(var c,d=0;d<b.result.files.length;d++)c=b.result.files[d],c._index=a,f.push(c);return b.result.files},
messages:{maxNumberOfFiles:gx.getMessage("GXM_fileupload_maxNumberOfFiles"),acceptFileTypes:gx.getMessage("GXM_fileupload_acceptFileTypes"),maxFileSize:gx.getMessage("GXM_fileupload_maxFileSize")},showCancelDownloadButton:this.EnableUploadedFileCanceling,autoDisableAdd:this.AutoDisableAddingFiles};0<this.MaxNumberOfFiles&&(l.maxNumberOfFiles=Number(this.MaxNumberOfFiles));r&&(l.acceptFileTypes=r);g.fileupload("option",l)}};this.destroy=function(){g&&(g.fileupload("destroy"),c.cleanData(g))};this.Clear=
function(){g.find("table tbody.files").empty();g.find("input[type\x3dfile]").removeAttr("disabled");f=[];a=0;b=[];h=0};this.StartUpload=function(){g.find(".start").click()}};
