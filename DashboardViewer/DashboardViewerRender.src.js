...
var MapType = { Choropleth: "Choropleth", Bubble: "Bubble" };
...
function DashboardViewer($) {
	...
	this.show = function () {
		...
		function tryToRenderDashboard() {
		
			qv.services.GetDashboardSpecIfNeeded(uc, function (resText, dViewer) {				// Servicio GetDashboardSpec
		...
		var uc = this;
		qv.util.dom.loadStylesAndScripts(this.getMainStyles(), [], [], tryToRenderDashboard);
	...
	this.ObjectDefaults = function (object, dashboard) {
		...
		object.ReturnSampleData = object.ReturnSampleData || false;
	}
	...
	this.FilterDefaults = function (filter, dashboard) {
		...
		filter.ReturnSampleData = filter.ReturnSampleData || false;
	...
	this.getMainStyles = function () {
		var styles = [];
		styles.push("DashboardViewer/DashboardViewer.css");
		styles.push("DashboardViewer/BrowserLayout.css");
		return styles;
	}
	...
	this.GetLayoutHTML = function (dashboard) {
		...
		var mainTableIds = [];
		var filtersTableIds = [];
		var htmlMainTable = this.editor.GetTableHTML(dashboard, dashboard.Layout.MainTable, false, mainTableIds, true, "gx-db-main-table");
		var htmlFiltersTable = this.editor.GetTableHTML(dashboard, dashboard.Layout.FiltersTable, false, filtersTableIds, true, dashboard.Layout.FiltersPosition === FiltersPosition.Top ? "gx-db-filters-table gx-db-horizontal-filters": "gx-db-filters-table gx-db-vertical-filters");
	...
	this.GetFilterValueClasses = function(filter) {
		var valueCls = filter.ValueClass == "" ? "gx-db-filter-value" : filter.ValueClass;
		switch (filter.Type) {
			case FilterType.Value:
				valueCls += " " + "gx-db-value-filter";
				break;
			case FilterType.Range:
				valueCls += " " + "gx-db-range-filter";
				break;
			case FilterType.Collection:
				valueCls += " " + "gx-db-collection-filter";
				break;
		}
		switch (filter.DataType) {
			case DataType.Integer:
			case DataType.Real:
				valueCls += " " + "gx-db-numeric-filter";
				break;
			case DataType.Character:
				valueCls += " " + "gx-db-character-filter";
				break;
			case DataType.Boolean:
				valueCls += " " + "gx-db-boolean-filter";
				break;
			case DataType.Date:
				valueCls += " " + "gx-db-date-filter";
				break;
			case DataType.DateTime:
				valueCls += " " + "gx-db-datetime-filter";
				break;
			case DataType.GUID:
				valueCls += " " + "gx-db-guid-filter";
				break;
			case DataType.GeoPoint:
				valueCls += " " + "gx-db-geopoint-filter";
				break;
		}
		return valueCls;
	}
	...
	this.GetFilterWidgetHTML = function (filter, variables) {
		...
		function RenderEditboxFilter(uc, filter, variables) {
			...
			var valueCls = uc.GetFilterValueClasses(filter);
		...
		function RenderSliderFilter(uc, filter, variables) {
			...
			function SliderFilterScripts () {
				return ["DashboardViewer/noUiSlider/nouislider.min.js"];
			}

			function SliderFilterStyles() {
				return ["DashboardViewer/noUiSlider/nouislider.min.css"];
			}
			
			var html = '';
			html += GetFilterCaptionHTML(uc, filter, variables, "", "");
			html += '<div id="' + uc.GetRangeElementId(filter.ElementId) + '"></div><div style="height:20px"></div>';
			qv.util.dom.loadStylesAndScripts(uc.getMainStyles(), SliderFilterStyles(), SliderFilterScripts(), DoRenderSliderFilter);
			return html;
		}
		...
		function RenderCalendarFilter(uc, filter, variables) {
			...
			function CalendarFilterScripts () {
				return ["DashboardViewer/DateRangePicker/moment.min.js", "DashboardViewer/DateRangePicker/daterangepicker.js"];
			}

			function CalendarFilterStyles() {
				return ["DashboardViewer/DateRangePicker/daterangepicker.css"];
			}
			...
			if (useDateRangePicker) {
				...
				qv.util.dom.loadStylesAndScripts(uc.getMainStyles(), CalendarFilterStyles(), CalendarFilterScripts(), DoRenderCalendarFilter);
			}
			else {
		...
		function RenderDropDownListFilter(uc, filter, variables) {
			...
			function DropDownListFilterScripts() {
				return ["DashboardViewer/bootstrap-select/bootstrap-select.min.js"];
			}

			function DropDownListFilterStyles() {
				return ["DashboardViewer/bootstrap-select/bootstrap-select.min.css"];
			}
			...
			html += '</select>';
			qv.util.dom.loadStylesAndScripts(uc.getMainStyles(), DropDownListFilterStyles(), DropDownListFilterScripts(), DoRenderDropDownListFilter);
			return html;
		}
		...
		function RenderSwitchFilter(uc, filter, variables) {
			...
			function SwitchFilterScripts () {
				return ["DashboardViewer/bootstrap-switch/bootstrap-switch.min.js"];
			}

			function SwitchFilterStyles() {
				return ["DashboardViewer/bootstrap-switch/bootstrap-switch.min.css"];
			}

			var html = '';
			html += '<div class="gx-db-switch-filter">';
			...
			html += '</div>';
			qv.util.dom.loadStylesAndScripts(uc.getMainStyles(), SwitchFilterStyles(), SwitchFilterScripts(), DoRenderSwitchFilter);
			return html;
		}
		...
		if (filter.Visible) {
			if (NeedToRefreshValues(filter)) {
				...
				var LoadControlValuesFn = qv.services.createAsyncServerCallFn(this.FakeQueryViewer(filter, queryName), qv.services.url.getDataURL, qv.services.postInfo.Data, "GetData");
	...
	this.RenderQueryViewer = function (widgetId) {
		...
		if (obj != null) {
			...
			qViewer.ReturnSampleData = obj.ReturnSampleData;
			...
			qViewer.ApplicationNamespace = this.ApplicationNamespace;
	...
	this.FakeQueryViewer = function (filter, queryName) {
		var dashboard = this.DashboardObject;
