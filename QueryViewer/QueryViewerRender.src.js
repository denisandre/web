...
function QueryViewer() {
	...
	this.show = function () {
		if (!gx.lang.gxBoolean(this.AvoidAutomaticShow) || gx.lang.gxBoolean(this.ExecuteShow)) {	// Used only in GXquery to avoid Show() function execution
			if (gx.lang.gxBoolean(this.Visible)) {
				...
				qv.TranslationType = this.TranslationType;		// all QueryViewers share the same value because it's an Environment property
	...
	this.GetMetadata = function () {
		switch (this.RealType) {
			...
			case QueryViewerOutputType.Map:
				return qv.map.GetMetadataMap(this);
			default:
	...
	this.GetFilteredData = function () {
		switch (this.RealType) {
			...
			case QueryViewerOutputType.Map:
				return qv.map.GetDataMap(this);			// en los Maps coincide el GetData con el GetFilteredData
			default:
	...
	this.NotifyMetadata = function () {
		...
		qv.util.ExecuteTracker("CollGeneXusReporting\QueryViewerElements.Element", metadata);
	}
	...
	this.NotifyData = function () {
		...
		qv.util.ExecuteTracker("GeneXusReporting\QueryViewerData", sdtdata);
	}
	...
	this.NotifyFilteredData = function () {
		...
		var sdtdata = {};
		sdtdata.XmlData = data;
		qv.util.ExecuteTracker("GeneXusReporting\QueryViewerFilteredData", sdtdata);
	}
	...
	this.realShow = function () {
		...
		this.calculateRealOutputType((function () {
			...
			function tryToRenderControl() {
				if (gx.lang.gxBoolean(qViewer.RememberLayout) && !qViewer._isAutorefresh && qViewer.AutoRefreshGroup != "")
					qv.util.autorefresh.LoadAxesAndParametersState(qViewer);
				if (qViewer.RealType == QueryViewerOutputType.Table || qViewer.RealType == QueryViewerOutputType.PivotTable)
					qv.pivot.tryToRenderPivotTable(qViewer);
				else if (qViewer.RealType == QueryViewerOutputType.Card)
					qv.card.tryToRenderCard(qViewer);
				else if (qViewer.RealType == QueryViewerOutputType.Map)
					qv.map.tryToRenderMap(qViewer);
				else
					qv.chart.tryToRenderChart(qViewer);
			}

			var container = document.getElementById(this.ContainerName);
			this.SetContainerSize(container);
			if (!this.CanRefreshControl()) {
				if (this._ControlRenderedTo === QueryViewerOutputType.Map)
					this.Charts[0].dispose();	// avoid initializing more than one ECharts instance in the same container (issue 90419)
			...
			qv.util.dom.loadStylesAndScripts(this.getMainStyles(), this.getLibrariesStyles(), this.getScripts(), tryToRenderControl);
		}).closure(this));
	...
	this.getScripts = function() {
		...
		switch (this.RealType) {
			case QueryViewerOutputType.PivotTable:
			case QueryViewerOutputType.Table:
				scripts.push("QueryViewer/oatPivot/gxpivotjs.js");
				break;
			case QueryViewerOutputType.Chart:
				scripts.push("QueryViewer/highcharts/highcharts.js");
				scripts.push("QueryViewer/highcharts/highcharts-3d.js");
				scripts.push("QueryViewer/highcharts/highcharts-more.js");
				scripts.push("QueryViewer/highcharts/modules/funnel.js");
				scripts.push("QueryViewer/highcharts/modules/solid-gauge.js");
				break;
			case QueryViewerOutputType.Map:
				scripts.push("QueryViewer/Echarts/dist/echarts.min.js");
				scripts.push(qv.map.getMapDirectory(this).mapDir);
				break;
			case QueryViewerOutputType.Card:
				if (this.IncludeSparkline)
					scripts.push("QueryViewer/highcharts/highcharts.js");
				break;
		}
		return scripts;
	...
	this.getMainStyles = function () {
		var styles = [];
		styles.push("QueryViewer/QueryViewer.css");
		return styles;
	}

	this.getLibrariesStyles = function () {

		var styles = [];
		switch (this.RealType) {
			case QueryViewerOutputType.PivotTable:
				styles.push("QueryViewer/oatPivot/css/pivot.css");
				styles.push("QueryViewer/oatPivot/css/common.css");
				styles.push("QueryViewer/oatPivot/css/winrect.css");
				break;
			case QueryViewerOutputType.Table:
				styles.push("QueryViewer/oatPivot/css/grid.css");
				styles.push("QueryViewer/oatPivot/css/common.css");
				styles.push("QueryViewer/oatPivot/css/winrect.css");
				break;
			case QueryViewerOutputType.Chart:
				styles.push("QueryViewer/highcharts/css/highcharts.css");
				break;
			case QueryViewerOutputType.Card:
				if (this.IncludeSparkline)
					styles.push("QueryViewer/highcharts/css/highcharts.css");
	...
	this.RealObjectName = function() {
		if (this.ObjectName != "")
			return this.ObjectName;
		else {
			if (this.QueryInfo != "" && qv.util.startsWith(this.QueryInfo, "{") && qv.util.endsWith(this.QueryInfo, "}")) {
				var queryInfo = JSON.parse(this.QueryInfo);
	...
	this.calculateRealOutputType = function (callback) {
		if (this.Type == QueryViewerOutputType.Default) {
			this.getDefaultOutput((function (output, qViewer) {
				if (output != QueryViewerOutputType.Table && output != QueryViewerOutputType.PivotTable && output != QueryViewerOutputType.Card && output != QueryViewerOutputType.Map) {
